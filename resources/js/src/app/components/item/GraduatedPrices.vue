<template>
    <div v-if="graduatedPrices[0]">
        <b>{{ $translate("Frontend::Template.singleItemGraduatedPrices") }}:</b>
        <table class="graduated-prices-table text-muted">
            <tbody>
                <template v-for="(price, index) in graduatedPrices">
                    <tr>
                        <td :class="paddingClasses" :style="paddingInlineStyles">{{ $translate("Frontend::Template.singleItemMinimumQuantity") }} {{ price.minimumOrderQuantity }}</td>
                        ...
                        <td v-if="showBasePrice" :class="paddingClasses" :style="paddingInlineStyles" class="graduated-base-price pl-3 d-none d-xl-block">{{ $translate("Frontend::Template.singleItemGraduatedBasePrice", { "price": price.basePrice }) }}</td>
                    </tr>
                    <tr v-if="showBasePrice" class="graduated-base-price d-xl-none">
                        <td :class="paddingClasses" :style="paddingInlineStyles"></td>
                        <td :class="paddingClasses" :style="paddingInlineStyles">{{ $translate("Frontend::Template.singleItemGraduatedBasePrice", { "price": price.basePrice }) }}</td>
                    </tr>
                </template>
            </tbody>
        </table>
