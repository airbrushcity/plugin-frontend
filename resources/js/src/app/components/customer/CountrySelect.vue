<template>
    <div>
        <div class="input-unit" data-validate="" data-model="countryId">
            ...
            <label :for="'country-id-select' + _uid">{{ $translate("Frontend::Template.headerCountry") }}</label>
        </div>
        ...
        <template v-if="isInOptionalFields('stateId')">
            <div
                ...
                <select :id="'state-id-select' + _uid" :value="selectedStateId" class="custom-select" @change="stateChanged($event.target.value)">
                    <option :selected="selectedStateId === null">{{ $translate("Frontend::Template.addressPleaseSelect") }}</option>
                    <option :value="state.id" :selected="state.id === selectedStateId" v-for="state in stateList" :key="state.id">
                        {{ state.name }}
                    </option>
                </select>
                <label :for="'state-id-select' + _uid">{{ transformTranslation("Frontend::Template.headerState", "stateId") }}</label>
            </div>
        </template>
    </div>
</template>
